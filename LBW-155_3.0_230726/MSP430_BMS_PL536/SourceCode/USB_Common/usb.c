...
#include "..\main.h"                      //add by Tei 120828
...
BYTE USB_init(VOID)
{
    ...
    /*USBPWRCTL   =     VUSBEN + SLDOEN + SLDOAON; // enable primary and secondary LDO (3.3 and 1.8 V)*/ /* delete PAN yoshino 2012.08.27 */
    USBPWRCTL   &= ~VUSBEN; // desable primary and secondary LDO (3.3 and 1.8 V)                         /* add PAN yoshino 2012.08.27 */
    USBPWRCTL   &= ~SLDOEN; // desable primary and secondary LDO (3.3 and 1.8 V)                         /* add PAN yoshino 2012.08.27 */
    USBPWRCTL   &= ~SLDOAON; // desable primary and secondary LDO (3.3 and 1.8 V)                        /* add PAN yoshino 2012.08.27 */
#if (LOW_POWER_MODE == LPM0_bits)
    {
    	volatile unsigned int i;
    	for (i =0; i < 1000; i++);       // wait some time for LDOs*/
    }

    USBPWRCTL   =   VUSBEN + SLDOEN + SLDOAON + VBONIE;  // enable interrupt VBUSon                  /* delete PAN yoshino 2012.08.27 */
#endif
    ...
    /*__enable_interrupt();                // enable global interrupts*/                                 /* delete PAN yoshino 2012.08.27 */
    return kUSB_succeed;
}
...
//        USBPLLCTL = UPCS0 + UPFDEN_H + UPLLEN_H;// Select XT2 as Ref / Select PLL for USB / Discrim. on, enable PLL
    }
    else
    {
        USBPLLCTL = UPFDEN + UPLLEN;        // Select XT1 as Ref / Select PLL for USB / Discrim. on, enable PLL
//        USBPLLCTL = UPFDEN_H + UPLLEN_H;        // Select XT1 as Ref / Select PLL for USB / Discrim. on, enable PLL
    }
