...
extern unsigned short LPM_F;
extern unsigned short Check_F;
extern unsigned short LV_F;
...
short bq_pack_init(void)
{
  ...
  for (i=0; i<NUMBER_OF_BQ_DEVICES -2 ; i++)
  {
  ...
  //--------------add 2017.07.07---------------//
  bq_pack.bq_devs[1].cell_count = 5;
  conf_bq_dev(&bq_pack.bq_devs[1]);
  init_bq_dev_data_structure(&bq_pack.bq_devs[1]);
  bq_dev_read_cell_voltage(&bq_pack.bq_devs[1]);

  bq_pack.bq_devs[2].cell_count = 5;
  conf_bq_dev(&bq_pack.bq_devs[2]);
  init_bq_dev_data_structure(&bq_pack.bq_devs[2]);
  bq_dev_read_cell_voltage(&bq_pack.bq_devs[2]);
...
{
  unsigned char i, cell_cnt, meas_cnt;
  unsigned long int stack_Cellvoltage[8][32];       // add 2021.10.04
  unsigned short stack_voltage = 0;
  //read the pack data
//  for (i=0; i<NUMBER_OF_BQ_DEVICES; i++)
//  {
  for (meas_cnt = 0; meas_cnt < 10; meas_cnt++)    ////////////////////////////////////////////////////////////--------------
  {
    for (i=0; i<NUMBER_OF_BQ_DEVICES; i++)
    {
      if (meas_cnt == 0) {
         ...
         {
        	 stack_Cellvoltage[i][cell_cnt] = 0;
         }
      }
...
///////////////////////////////////////////////////////////////////////////////
      ...
      for (cell_cnt=0; cell_cnt<bq_pack.bq_devs[i].cell_count; cell_cnt++)
      {
        stack_Cellvoltage[i][cell_cnt] += bq_pack.bq_devs[i].cell_voltage[cell_cnt];
      }
    }
  }
  ...
  for (i=0; i<NUMBER_OF_BQ_DEVICES; i++)
  {
    for (cell_cnt=0; cell_cnt<bq_pack.bq_devs[i].cell_count; cell_cnt++)
    {
    	bq_pack.bq_devs[i].cell_voltage[cell_cnt] = stack_Cellvoltage[i][cell_cnt] / 10;    /////////////////////////////////////----------
    }
    ...
    for (cell_cnt=0; cell_cnt<bq_pack.bq_devs[i].cell_count; cell_cnt++)
    {
      ...
      if (bq_pack.bq_devs[i].cell_voltage[cell_cnt] < bq_pack.lowest_cell_volts)
      {
...
void CheckFaultConditions(void)
{
  ...
  {
//    bq_pack.error_status |= STATUS_ERROR_POT_COV; //del 20230614 エラー無視

    //can only return to previous mode when error condition disapears
  //  update_op_mode(FAULT_MODE);            // del 2017.06.19
   // update_op_mode(END_OF_CHARGE);         //add 2017.06.19
  }
  //Check for CUV condition
  if (check_for_cuv())
  {
//    bq_pack.error_status |= STATUS_ERROR_CUV; //del 20230331 エラー無視

    //can only return to previous mode when error condition disapears
 //   update_op_mode(FAULT_MODE);          // del 2017.06.19
   // update_op_mode(END_OF_DISCHARGE);   //add 2017.06.19
  }
...
void CellBalancing(void)
{
  ...
  if (bq_pack.lowest_cell_volts<get_u32_value(LOW_POWER_THERESHOLD)) {
	  LPM_F = 1;
  }
  else{
	  LPM_F = 0;
	  ...
	  if (bq_pack.timer_status & START_CELL_BALANCE_TIMER)
	  {
	    ...
	    if (bq_pack.op_mode <= SOV_MODE /*& bq_pack.op_mode > INITIAL_MODE*/ | HOST_CONTROL_IN & IN_HOST_CHG_OP)
...
void update_bq_pack_timer(void)
{
  ...
  if (bq_pack.lowest_cell_volts>get_u32_value(LOW_POWER_THERESHOLD)+20) {
  if (bq_pack.timer_status & START_CELL_BALANCE_TIMER)
  {
...
void conf_bq_dev(bq_dev_t* this)
{
  ...
  bq_dev_write_reg(this->device_address, ADC_CONTROL_REG, ADC_CONTROL_VAL_6);    // mod 2017.04.17  4 -> 6
  ...
  bq_dev_write_reg(this->device_address, FUNCTION_CONFIG_REG, FUNC_CONFIG_VAL_5);   // mod 2017.07.07  6 -> 5
  
  bq_dev_write_reg(this->device_address, SHDW_CTRL_REG, SHDW_CTRL_ACCESS_EN_VAL);
  bq_dev_write_reg(this->device_address, IO_CONFIG_REG, IO_CONFIG_VAL);


//過電圧FAULTにならないように temp = 50; (5V)で固定////////////////////////////
...
//LMP4を解除させないように temp = 0; で固定////////////////////////////
////  temp = get_u32_value(CUV_THRESHOLD);  //20230401
//  temp = get_u32_value(CUV_THRESHOLD);  //20230401
//  if (temp > 700)
//    temp = (temp - 700)/100;
//  else
...
void init_bq_dev_data_structure(bq_dev_t* this)
{
  ...
  bq_dev_read_reg(this->device_address, TEMPERATURE1_L_REG, 2, DISCARD_CRC,    // mod  1->2  20170430
...
/**
* @brief Function Name: bq_dev_clear_s .
* @brief Description  : Clear the  flags on the BQ device.
  ...
  for (i=0; i<NUMBER_OF_BQ_DEVICES; i++)
  {
    if (bq_pack.bq_devs[i].fault_status & (1<<FAULT_COV_POS))
    {
      ...
      for (cell=0; cell < bq_pack.bq_devs[i].cell_count; cell++)
      {
        ...
        {
                //  OV to LED ON    by S.tei  120629
     /*           bq_dev_write_reg(bq_pack.bq_devs[i].device_address,
                                   IO_CONTROL_REG, (1<<6)|(1<<1)|(1<<0)); */
        //        BQ76PL536_OV_PxOUT |= OUT_BQ_OV;
          ...
          {
                //  OV to LED OFF    by S.tei  120629
          /*        bq_dev_write_reg(bq_pack.bq_devs[i].device_address,
                                     IO_CONTROL_REG, (0<<6)|(1<<1)|(1<<0)); */
                //  BQ76PL536_OV_PxOUT &= ~OUT_BQ_OV;
            ...
            update_op_mode(INITIAL_MODE);   //del 2017.06.20
          }
        }
      }
    }
...
//            return 1; //error　　　　　　　　　　//2030426 エラー回避
          }
          else
          {

            //  UV to LED OFF    by S.tei  120629
                 bq_dev_write_reg(bq_pack.bq_devs[i].device_address,
                                     IO_CONTROL_REG, (0<<6));
...
//////////////// UV to output (add 2021.05.18 H.Yuri) ////////////////
 

   //20230525追加  /////////////////////////////////////////////////
   //UV閾値以上なら、必ず、フラグ0、TEMP_PxOUTの出力OFF にする
    if (bq_pack.lowest_cell_volts>get_u32_value(CUV_THRESHOLD)) {
    	bq_pack.uv_output_hold_F = 0;
    	BQ76PL536_TEMP_PxOUT &= ~OUT_BQ_TEMP;
    }
    ///////////////////////////////////////////////////////////////


    if (bq_pack.uv_output_hold_F > 1) bq_pack.uv_output_hold_F = 0;
    if (bq_pack.lowest_cell_volts<=get_u32_value(CUV_THRESHOLD)) {
/*      bq_pack.uv_output_delay_timer++;
    }
    else {
      bq_pack.uv_output_delay_timer=0;
    }
  	if ((bq_pack.uv_output_hold_F==0)&&(bq_pack.uv_output_delay_timer>=2)) {  /////////////////////////////////////////////////////////////
*/
    	bq_pack.uv_output_hold_F = 1;
        bq_pack.uv_output_hold_timer = 0;
        bq_pack.uv_output_delay_timer = 0;
    }
    if (bq_pack.uv_output_hold_F == 1) {
        bq_pack.uv_output_hold_timer++ ;
        BQ76PL536_TEMP_PxOUT |= OUT_BQ_TEMP;
      if (bq_pack.uv_output_hold_timer==15) {
           BQ76PL536_TEMP_PxOUT &= ~OUT_BQ_TEMP;
//           bq_pack.uv_output_hold_timer = 10;
           bq_pack.uv_output_hold_timer = 0;
     	   bq_pack.uv_output_hold_F = 0;
      }
//      else if (bq_pack.uv_output_hold_timer>10) {
//           bq_pack.uv_output_hold_timer = 10;
//      }
//      if (bq_pack.lowest_cell_volts>=get_u32_value(CUV_RECOVERY_THRESHOLD)) {
//      	    bq_pack.uv_output_hold_F = 0;
//            bq_pack.uv_output_hold_timer = 0;
//      }
...
//////////////// OV to output (add 2021.05.18 H.Yuri) ////////////////
    if (bq_pack.highest_cell_volts>=get_u32_value(COV_THRESHOLD) & bq_pack.highest_cell_volts<=5000){
            bq_pack.ov_output_delay_timer++ ;
    }
    else {
            bq_pack.ov_output_delay_timer= 0;
    }
    if (bq_pack.ov_output_delay_timer>=3) {
        if ((bq_pack.highest_cell_volts - bq_pack.lowest_cell_volts)
             > get_u32_value(CELL_IMBALANCE_FAULT_V)){
                 BQ76PL536_TEMP_PxOUT |= OUT_BQ_TEMP;
                 BQ76PL536_OV_PxOUT |= OUT_BQ_OV;
        }
        else{
    ...
    else if (bq_pack.highest_cell_volts<=get_u32_value(COV_RECOVERY_THRESHOLD)) {
            BQ76PL536_TEMP_PxOUT &= ~OUT_BQ_TEMP;
            BQ76PL536_OV_PxOUT &= ~OUT_BQ_OV;
    }
    //////////////////////////////////////////////////////////////////////
