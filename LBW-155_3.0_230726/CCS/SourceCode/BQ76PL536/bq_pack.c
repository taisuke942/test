...
*
*  @author S.tei & H.mochizuki Advanced capacitor technologies, Inc
*  @date September 2012
*  @version 1.2  version
...
void CellBalancing(void)
{
  ...
  if (bq_pack.timer_status & START_CELL_BALANCE_TIMER)
  {
    ...
    if (bq_pack.op_mode < END_OF_CHARGE & bq_pack.op_mode > INITIAL_MODE | HOST_CONTROL_IN & IN_HOST_CHG_OP)
    { 
...
void conf_bq_dev(bq_dev_t* this)
{
  ...
  bq_dev_write_reg(this->device_address, ADC_CONTROL_REG, ADC_CONTROL_VAL_4);
  ...
  bq_dev_write_reg(this->device_address, FUNCTION_CONFIG_REG, FUNC_CONFIG_VAL_4);
...
void init_bq_dev_data_structure(bq_dev_t* this)
{
  ...
  bq_dev_read_reg(this->device_address, TEMPERATURE1_L_REG, 1, DISCARD_CRC,
...
short check_for_cuv(void)
{
  ...
  for (i=0; i<NUMBER_OF_BQ_DEVICES; i++)
  {
	...
	// OT to LED OFF by S.tei 120703
	  if (bq_pack.bq_devs[0].alert_status == (0<<1 | 0<<0)){
	   	  BQ76PL536_TEMP_PxOUT &= ~OUT_BQ_TEMP;
	  }
		for (i=0; i<NUMBER_OF_BQ_DEVICES; i++){
		// OV to output by S.tei 120717
			if (bq_pack.bq_devs[i].fault_status & BIT0) {
				BQ76PL536_OV_PxOUT |= OUT_BQ_OV;
			}
			else{
				BQ76PL536_OV_PxOUT &= ~OUT_BQ_OV;
			}
			// FAULT to output by S.tei 120717
		//	if (bq_pack.bq_devs[i].device_status & BIT6)
			if (bq_pack.bq_devs[i].alert_status & BIT7 | bq_pack.bq_devs[i].alert_status & BIT6 |
			    bq_pack.bq_devs[i].alert_status & BIT5 | bq_pack.bq_devs[i].alert_status & BIT4 |
			    bq_pack.bq_devs[i].alert_status & BIT3 | bq_pack.bq_devs[i].fault_status & BIT5 |
			    bq_pack.bq_devs[i].fault_status & BIT4 | bq_pack.bq_devs[i].fault_status & BIT3 |
			    bq_pack.bq_devs[i].fault_status & BIT2 | bq_pack.highest_cell_volts - bq_pack.lowest_cell_volts >=dCELL_IMBALANCE_FAIL_THRESHOLD)
			{
      ...
      /*        #if (LOW_POWER_MODE==LPM3_bits)
	         if (BQ76PL536_LED_PxOUT & OUT_BQ_LED){
			//   if ((P2OUT & BIT1)&(BQ76PL536_LED_PxOUT & OUT_BQ_LED)){
			//   if (P1OUT & BIT5){
			//if (TA1R > 49152){
		    	bq_dev_write_reg(bq_pack.bq_devs[i].device_address,
		    	      IO_CONTROL_REG, (0<<6)|(0<<1)|(1<<0));
		    	bq_dev_write_reg(bq_pack.bq_devs[i].device_address,
		    	      ADC_CONTROL_REG, (0<<6)|(1<<4)|(0<<3)|(3<<0));}
		    else {	bq_dev_write_reg(bq_pack.bq_devs[i].device_address,
		    	      IO_CONTROL_REG, (0<<6)|(0<<1)|(0<<0));
		    	bq_dev_write_reg(bq_pack.bq_devs[i].device_address,
		    	      ADC_CONTROL_REG, (0<<6)|(0<<4)|(0<<3)|(3<<0));}
              #endif */
		}
  return 0;
}
...
{
	// OT to LED ON by S.tei 120703
	  if ((bq_pack.bq_devs[0].alert_status & BIT1)|(bq_pack.bq_devs[0].alert_status & BIT0)){
	   BQ76PL536_TEMP_PxOUT |= OUT_BQ_TEMP;
	  }
	  // OT to LED OFF  by S.tei 120703
	/*  if (bq_pack.bq_devs[0].alert_status < 1)
	   {
		  BQ76PL536_TEMP_PxOUT &= ~OUT_BQ_TEMP;
	  }*/
